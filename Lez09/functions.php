...
function addLibro($libro){
    ...
    $titolo = mysqli_escape_string($conn, $libro["titolo"]);
    $pagine = mysqli_escape_string($conn, $libro["pagine"]);
    $prezzo = mysqli_escape_string($conn, $libro["prezzo"]);
    $autore = mysqli_escape_string($conn, $libro["autore"]);
    
    if(empty(trim($titolo))){
        return errore("422", "Devi inserire un titolo");
    }elseif(empty(trim($autore))){
        return errore("422", "Devi inserire un autore!");
    }else {
